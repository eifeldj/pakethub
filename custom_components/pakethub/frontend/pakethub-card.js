...
class PaketHubCard extends HTMLElement {
  ...
  _render(supplied) {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      @media(max-width:520px){.modal-backdrop{padding:0;place-items:stretch}.detail-modal{width:100%;height:100dvh;max-height:100dvh;border-radius:0}.detail-modal header{padding:13px 12px}.detail-status{max-width:34%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.detail-content{padding:14px;max-height:calc(100dvh - 68px);padding-bottom:max(18px,env(safe-area-inset-bottom))}.detail-actions{flex-direction:column}.timeline-head{display:block}.timeline-head time{display:block;margin-top:4px}.header{padding:16px 14px 12px}.summary{grid-template-columns:repeat(2,1fr)}.content{padding:8px}.package{padding:14px}.info-grid,.progress-row,.event{margin-left:0}.chip{max-width:38%;padding:5px 7px}}
