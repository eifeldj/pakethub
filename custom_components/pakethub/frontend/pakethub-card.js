...
class PaketHubCard extends HTMLElement {
  ...
  _packages() {
    ...
    if (this._query) { const q=this._query.toLowerCase(); list=list.filter(p => `${p.package_name||""} ${p.tracking_number||""} ${p.carrier||""} ${p.latest_event||""}`.toLowerCase().includes(q)); }
  ...
  _render(supplied) {
    ...
    const body = packages.length ? packages.map((item,index) => {
      ...
      return `<article class="package ${meta.cls} ${expanded?"expanded":""}" data-index="${index}" tabindex="0" role="button" aria-expanded="${expanded}" aria-label="${this._escape(item.package_name || item.tracking_number || "Package")}: ${this._escape(meta.label)}">
