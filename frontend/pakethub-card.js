...
class PaketHubCard extends HTMLElement {
  ...
  _render(supplied) {
    ...
    const body = packages.length ? packages.map((item,index) => {
      ...
      return `<article class="package ${meta.cls}" data-index="${index}" tabindex="0" role="button" aria-label="${this._escape(item.package_name || item.tracking_number || "Package")}: ${this._escape(meta.label)}">
        ...
        <div class="package-main">
          <div class="topline"><div class="heading"><div class="name-row"><div class="name">${this._escape(item.package_name || item.tracking_number || "Package")}</div><div class="tracking-tools">${suffix}${copy}</div></div><div class="carrier">${this._escape(item.carrier || this._t("carrier_unknown"))}</div></div><div class="chip"><i></i>${this._escape(meta.label)}</div></div>
          ...
          <div class="info-grid"><div class="info-block eta-block"><span>${this._escape(this._t("eta"))}</span><strong><ha-icon icon="mdi:calendar-clock-outline"></ha-icon>${this._escape(this._eta(item))}</strong></div>${location}</div>${event}
        </div>
      </article>`;
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .carrier-brand{min-height:210px;padding:24px 16px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;background:var(--carrier-bg);color:var(--carrier-fg);border-right:1px solid color-mix(in srgb,var(--divider-color) 82%,transparent);box-sizing:border-box}.carrier-mark{width:100%;height:78px;display:grid;place-items:center}.carrier-mark svg{display:block;width:100%;height:100%;max-width:132px;overflow:visible}.carrier-brand>span{font-size:13px;font-weight:650;text-align:center;color:color-mix(in srgb,var(--carrier-fg) 82%,var(--primary-text-color) 18%)}.generic-logo{display:grid;place-items:center;gap:8px;text-align:center}.generic-logo ha-icon{--mdc-icon-size:36px}.generic-logo strong{font-size:15px;overflow-wrap:anywhere}.package-main{min-width:0;padding:25px 30px 24px}.topline{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:start;gap:14px}.heading{min-width:0}.name-row{display:flex;align-items:flex-start;gap:9px;min-width:0}.name{font-size:21px;font-weight:700;line-height:1.22;white-space:normal;overflow-wrap:anywhere;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;min-width:0}.tracking-tools{display:flex;align-items:center;gap:4px;flex:0 0 auto;padding-top:1px}.suffix{font-size:13px;color:var(--secondary-text-color);white-space:nowrap}.copy-inline{width:32px;height:32px;border:0;border-radius:9px;display:grid;place-items:center;background:transparent;color:var(--secondary-text-color);cursor:pointer;flex:0 0 auto}.copy-inline:hover{background:var(--secondary-background-color);color:var(--primary-text-color)}.copy-inline ha-icon{--mdc-icon-size:18px}.carrier{margin-top:7px;font-size:14px;color:var(--secondary-text-color);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
