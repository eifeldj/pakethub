...
const CARRIER_META = [
  { match: /austrian post|österreichische post|post at|post\.at/i, key: "postat", short: "Post", name: "Austrian Post", bg: "#17191d", fg: "#ffd400" },
  { match: /dhl/i, key: "dhl", short: "DHL", name: "DHL", bg: "#ffcc00", fg: "#d40511" },
  { match: /ups/i, key: "ups", short: "UPS", name: "UPS", bg: "#351c15", fg: "#ffb500" },
  { match: /fedex/i, key: "fedex", short: "FedEx", name: "FedEx", bg: "#ffffff", fg: "#4d148c" },
  { match: /gls/i, key: "gls", short: "GLS", name: "GLS", bg: "#061ab1", fg: "#ffdf00" },
  { match: /dpd/i, key: "dpd", short: "DPD", name: "DPD", bg: "#ffffff", fg: "#dc0032" },
  { match: /amazon/i, key: "amazon", short: "amazon", name: "Amazon Logistics", bg: "#131a22", fg: "#ff9900" },
  { match: /yunexpress|yun express/i, key: "yun", short: "YunExpress", name: "YunExpress", bg: "#162126", fg: "#00a2b8" },
  { match: /hermes|evri/i, key: "hermes", short: "Hermes", name: "Hermes / Evri", bg: "#ffffff", fg: "#0091cd" },
  { match: /royal mail/i, key: "royalmail", short: "Royal Mail", name: "Royal Mail", bg: "#e4002b", fg: "#ffffff" },
  { match: /usps|united states postal/i, key: "usps", short: "USPS", name: "USPS", bg: "#ffffff", fg: "#004b87" },
];
...
class PaketHubCard extends HTMLElement {
  ...
  _carrierMeta(name) {
    ...
    return CARRIER_META.find(item => item.match.test(carrier)) || {key:"generic", short:(carrier.trim() || "Carrier"), name:(carrier.trim() || this._t("carrier_unknown")), bg:"var(--secondary-background-color)", fg:"var(--primary-text-color)"};
  }
  ...
  _render(supplied) {
    ...
    const body = packages.length ? packages.map((item,index) => {
      ...
      return `<article class="package ${meta.cls}" data-index="${index}" tabindex="0" role="button" aria-label="${this._escape(item.package_name || item.tracking_number || "Package")}: ${this._escape(meta.label)}">
        <aside class="carrier-brand" style="--carrier-bg:${carrierMeta.bg};--carrier-fg:${carrierMeta.fg}" title="${this._escape(item.carrier || this._t("carrier_unknown"))}">
          <div class="carrier-mark">${this._carrierLogo(carrierMeta)}</div><span>${this._escape(carrierMeta.name || item.carrier || this._t("carrier_unknown"))}</span>
        </aside>
        <div class="package-main">
          <div class="topline"><div class="heading"><div class="name-row"><div class="name">${this._escape(item.package_name || item.tracking_number || "Package")}</div>${suffix}${copy}</div><div class="carrier">${this._escape(item.carrier || this._t("carrier_unknown"))}</div></div><div class="chip"><i></i>${this._escape(meta.label)}</div></div>
          <div class="progress-row"><div class="progress"><div style="width:${progress}%"></div></div><div class="percent">${progress}%</div><ha-icon class="chevron" icon="mdi:chevron-right" aria-hidden="true"></ha-icon></div>
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .content{padding:18px;display:grid;gap:14px}.package{--status-color:var(--primary-color);width:100%;border:1px solid color-mix(in srgb,var(--divider-color) 76%,transparent);border-radius:20px;padding:0;text-align:left;cursor:pointer;color:var(--primary-text-color);background:linear-gradient(110deg,color-mix(in srgb,var(--card-background-color) 94%,var(--status-color) 6%),color-mix(in srgb,var(--card-background-color) 98%,#000 2%));font:inherit;display:grid;grid-template-columns:170px minmax(0,1fr);overflow:hidden;transition:transform .18s,box-shadow .18s,border-color .18s}.package:hover{transform:translateY(-1px);box-shadow:0 8px 22px rgba(0,0,0,.12)}.package:focus-visible{outline:2px solid var(--primary-color);outline-offset:2px}
      ...
      .carrier-brand{min-height:190px;padding:24px 18px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;background:var(--carrier-bg);color:var(--carrier-fg);border-right:1px solid color-mix(in srgb,var(--divider-color) 82%,transparent);box-sizing:border-box}.carrier-mark{width:100%;height:78px;display:grid;place-items:center}.carrier-mark svg{display:block;width:100%;height:100%;max-width:150px;overflow:visible}.carrier-brand>span{font-size:13px;font-weight:650;text-align:center;color:color-mix(in srgb,var(--carrier-fg) 82%,var(--primary-text-color) 18%)}.generic-logo{display:grid;place-items:center;gap:8px;text-align:center}.generic-logo ha-icon{--mdc-icon-size:36px}.generic-logo strong{font-size:15px;overflow-wrap:anywhere}.package-main{min-width:0;padding:26px 34px 24px 30px}.topline{display:flex;align-items:flex-start;gap:18px}.heading{min-width:0;flex:1}.name-row{display:flex;align-items:center;gap:9px}.name{font-size:21px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.suffix{font-size:13px;color:var(--secondary-text-color)}.copy-inline{width:32px;height:32px;border:0;border-radius:9px;display:grid;place-items:center;background:transparent;color:var(--secondary-text-color);cursor:pointer;flex:0 0 auto}.copy-inline:hover{background:var(--secondary-background-color);color:var(--primary-text-color)}.copy-inline ha-icon{--mdc-icon-size:18px}.carrier{margin-top:5px;font-size:14px;color:var(--secondary-text-color);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      ...
      .progress-row{margin:22px 0 16px;display:flex;align-items:center;gap:14px}.progress{height:8px;flex:1;border-radius:999px;overflow:hidden;background:color-mix(in srgb,var(--divider-color) 76%,transparent)}.progress>div{height:100%;border-radius:inherit;background:var(--status-color);transition:width .35s}.percent{min-width:40px;text-align:right;font-size:14px;font-weight:750;color:var(--primary-text-color)}.chevron{--mdc-icon-size:25px;color:var(--secondary-text-color);margin-left:4px}.info-grid{display:flex;align-items:center;gap:32px;flex-wrap:wrap}.eta-block{min-width:145px;padding:12px 14px;border-radius:14px;background:color-mix(in srgb,var(--secondary-background-color) 86%,transparent)}.eta-block span{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.7px;color:var(--secondary-text-color);margin-bottom:5px}.eta-block strong{display:flex;align-items:center;gap:7px;font-size:14px}.eta-block ha-icon,.detail ha-icon{--mdc-icon-size:18px;color:var(--status-color)}.detail{display:flex;align-items:center;gap:7px;font-size:14px;color:var(--secondary-text-color)}.event{margin:16px 42px 0 0;color:var(--secondary-text-color);font-size:14px;line-height:1.45;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
      ...
      @media(max-width:760px){.content{padding:10px}.package{grid-template-columns:1fr}.carrier-brand{min-height:auto;padding:13px 16px;flex-direction:row;justify-content:flex-start;border-right:0;border-bottom:1px solid color-mix(in srgb,var(--divider-color) 82%,transparent)}.carrier-mark{width:92px;height:42px;flex:0 0 auto}.carrier-mark svg{max-width:92px}.carrier-brand>span{font-size:12px;text-align:left}.package-main{padding:17px 18px 18px}.name{font-size:18px}.chip{max-width:42%;padding:6px 9px}.progress-row{margin:17px 0 13px;gap:9px}.chevron{display:none}.info-grid{gap:14px}.event{margin:13px 0 0}.eta-block{min-width:120px}.modal-backdrop{padding:0;place-items:stretch}.detail-modal{width:100%;height:100dvh;max-height:100dvh;border-radius:0}.detail-modal header{padding:13px 12px}.detail-status{max-width:34%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.detail-content{padding:14px;max-height:calc(100dvh - 68px);padding-bottom:max(18px,env(safe-area-inset-bottom))}.detail-actions{flex-direction:column}.timeline-head{display:block}.timeline-head time{display:block;margin-top:4px}.header{padding:16px 14px 12px}.summary{grid-template-columns:repeat(2,1fr)}}@media(max-width:420px){.topline{gap:10px}.chip{font-size:10px;max-width:37%}.name-row{gap:5px}.suffix{display:none}.copy-inline{width:28px;height:28px}.info-grid{display:grid;grid-template-columns:1fr}.eta-block{min-width:0}.carrier-brand>span{display:none}}
    </style><ha-card><div class="header"><div class="header-top"><div class="brand"><div class="brand-icon"><ha-icon icon="mdi:package-variant-closed"></ha-icon></div><div><div class="title">${this._escape(this._config.title || "PaketHub")}</div><div class="version">Dashboard v${CARD_VERSION}</div></div></div><button type="button" aria-label="${this._escape(this._t("refresh"))}" class="refresh ${this._refreshing ? "refreshing" : ""}" ${this._refreshing ? "disabled" : ""}><ha-icon icon="mdi:refresh"></ha-icon></button></div>
