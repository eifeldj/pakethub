...
class PaketHubCard extends HTMLElement {
  ...
  _packages() {
    ...
    else if (this._config.sort_by === "updated") list.sort((a,b) => this._eventTimestamp(b) - this._eventTimestamp(a));
    else if (this._config.sort_by === "name") list.sort((a,b) => (a.package_name || "").localeCompare(b.package_name || "", undefined, { sensitivity:"base" }));
    else if (this._config.sort_by === "carrier") list.sort((a,b) => (a.carrier || "").localeCompare(b.carrier || "", undefined, { sensitivity:"base" }));
  ...
  _eventTimestamp(item) {
    const value = item?.latest_event_time || item?.last_event_time || item?.updated_at || item?.last_update || "";
  ...
  _render(supplied) {
    ...
    const body = packages.length ? packages.map((item,index) => {
      ...
      const showAll=this._timelineAll.has(number);
      const visibleTimeline=showAll?timeline:timeline.slice(0,3);
      const timelineEvents=visibleTimeline.length?visibleTimeline.map((ev,i)=>{
        const provider=ev?.provider?`<small>${this._escape(ev.provider)}</small>`:"";
        const location=ev?.location?`<span class="event-location"><ha-icon icon="mdi:map-marker-outline"></ha-icon>${this._escape(ev.location)}</span>`:"";
        return `<div class="inline-event ${i===0?"current":""}"><i></i><div><strong>${this._escape(ev.description||this._t("status_update"))}</strong><span>${this._escape(this._formatDate(ev.time))}</span>${location}${provider}</div></div>`;
      }).join(""):`<div class="no-events">${this._escape(this._t("no_events"))}</div>`;
      const timelineToggle=timeline.length>3?`<button type="button" class="timeline-more" data-action="timeline-more" data-index="${index}" aria-expanded="${showAll}"><ha-icon icon="mdi:chevron-${showAll?"up":"down"}"></ha-icon>${this._escape(this._t(showAll?"show_less_events":"show_all_events"))}<span>${timeline.length}</span></button>`:"";
      const inlineTimeline=expanded?`<div class="inline-details"><div class="inline-section-title"><ha-icon icon="mdi:timeline-clock-outline"></ha-icon>${this._escape(this._t("latest_events"))}</div><div class="inline-timeline">${timelineEvents}</div>${timelineToggle}<div class="inline-actions"><button data-action="favorite" data-index="${index}"><ha-icon icon="mdi:star${isFav?"":"-outline"}"></ha-icon>${this._escape(this._t("favorites"))}</button><button data-action="rename" data-index="${index}"><ha-icon icon="mdi:pencil-outline"></ha-icon>${this._escape(this._t("rename"))}</button><button class="danger" data-action="remove" data-index="${index}"><ha-icon icon="mdi:delete-outline"></ha-icon>${this._escape(this._t("remove"))}</button></div></div>`:"";
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .summary{margin-top:14px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.summary-item{border:1px solid transparent;padding:9px 8px;border-radius:12px;background:var(--secondary-background-color);color:var(--primary-text-color);text-align:center;font:inherit;cursor:pointer;transition:border-color .16s,transform .16s,background .16s}.summary-item:hover{transform:translateY(-1px);border-color:var(--divider-color)}.summary-item.active{border-color:var(--primary-color);background:color-mix(in srgb,var(--primary-color) 12%,var(--secondary-background-color))}.summary-value{display:block;font-size:17px;font-weight:700}.summary-label{display:block;font-size:11px;color:var(--secondary-text-color)}.last-sync{margin-top:10px;font-size:12px;color:var(--secondary-text-color)}.toolbar{display:flex;gap:10px;align-items:center;margin-top:12px}.search{min-width:0;flex:1}.sort-control{min-height:42px;display:flex;align-items:center;gap:6px;padding:0 10px;border:1px solid var(--divider-color);border-radius:11px;background:var(--secondary-background-color)}.sort-control ha-icon{--mdc-icon-size:18px;color:var(--secondary-text-color)}.sort-control select{max-width:150px;border:0;outline:0;background:transparent;color:var(--primary-text-color);font:inherit;cursor:pointer}
    ...
    .header-actions{display:flex;gap:4px}.add-package,.favorites-filter,.inline-actions button,.form-actions button{border:0;background:transparent;color:var(--primary-text-color);font:inherit;cursor:pointer}.add-package{width:44px;height:44px;border-radius:50%;display:grid;place-items:center}.add-package:hover{background:var(--secondary-background-color)}.toolbar{display:flex;gap:10px;margin-top:14px}.search{flex:1;display:flex;align-items:center;gap:8px;padding:0 12px;border-radius:12px;background:var(--secondary-background-color)}.search ha-icon{--mdc-icon-size:19px;color:var(--secondary-text-color)}.search input{width:100%;height:42px;border:0;outline:0;background:transparent;color:var(--primary-text-color);font:inherit}.favorites-filter{display:flex;align-items:center;gap:6px;padding:0 13px;border-radius:12px;background:var(--secondary-background-color)}.favorites-filter.active{color:#ffc107;background:color-mix(in srgb,#ffc107 15%,var(--secondary-background-color))}.filter-row{display:flex;flex-wrap:wrap;gap:7px;margin-top:10px}.status-filter,.favorites-filter{min-height:36px;border:0;display:flex;align-items:center;justify-content:center;gap:6px;padding:0 11px;border-radius:999px;background:var(--secondary-background-color);color:var(--secondary-text-color);font:inherit;font-size:12px;font-weight:650;cursor:pointer}.status-filter:hover,.favorites-filter:hover{color:var(--primary-text-color)}.status-filter.active{color:var(--primary-text-color);background:color-mix(in srgb,var(--primary-color) 20%,var(--secondary-background-color));box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--primary-color) 45%,transparent)}.status-filter ha-icon,.favorites-filter ha-icon{--mdc-icon-size:17px}.delivered-filter span{min-width:18px;padding:1px 5px;border-radius:999px;background:color-mix(in srgb,var(--success-color,#43a047) 20%,transparent);color:var(--success-color,#43a047);font-size:10px}.delivered-filter.active{color:var(--success-color,#43a047);background:color-mix(in srgb,var(--success-color,#43a047) 16%,var(--secondary-background-color));box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--success-color,#43a047) 42%,transparent)}.favorite-inline{border:0;background:transparent;color:#ffc107;padding:0;display:grid;place-items:center;cursor:pointer}.favorite-inline ha-icon{--mdc-icon-size:20px}.package.expanded{grid-template-rows:auto}.inline-details{grid-column:1/-1;margin-top:18px;padding-top:16px;border-top:1px solid var(--divider-color);animation:pakethub-expand .2s ease-out}.inline-section-title{display:flex;align-items:center;gap:7px;margin-bottom:13px;font-size:12px;font-weight:750;color:var(--secondary-text-color);text-transform:uppercase;letter-spacing:.04em}.inline-section-title ha-icon{--mdc-icon-size:18px;color:var(--carrier-accent)}.inline-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px}.inline-actions button{display:flex;align-items:center;gap:6px;padding:8px 11px;border-radius:10px;background:var(--secondary-background-color)}.inline-actions .danger{color:var(--error-color,#db4437)}.inline-event{position:relative;display:grid;grid-template-columns:18px 1fr;gap:9px;padding-bottom:14px}.inline-event:not(:last-child):before{content:"";position:absolute;left:5px;top:13px;bottom:0;width:2px;background:var(--divider-color)}.inline-event i{width:10px;height:10px;margin-top:5px;border-radius:50%;background:var(--carrier-accent);z-index:1}.inline-event strong,.inline-event span,.inline-event small{display:block}.inline-event strong{font-size:13px;line-height:1.35}.inline-event span{font-size:11px;color:var(--secondary-text-color);margin-top:4px}.inline-event small{font-size:10px;color:var(--secondary-text-color);margin-top:3px;opacity:.8}.inline-event.current i{box-shadow:0 0 0 4px color-mix(in srgb,var(--carrier-accent) 18%,transparent)}.event-location{display:flex!important;align-items:center;gap:3px}.event-location ha-icon{--mdc-icon-size:13px}.timeline-more{width:100%;min-height:38px;margin-top:2px;border:0;border-radius:10px;display:flex;align-items:center;justify-content:center;gap:6px;background:var(--secondary-background-color);color:var(--primary-text-color);font:inherit;font-size:12px;font-weight:650;cursor:pointer}.timeline-more ha-icon{--mdc-icon-size:18px}.timeline-more span{min-width:20px;padding:1px 5px;border-radius:999px;background:color-mix(in srgb,var(--carrier-accent) 15%,transparent);color:var(--carrier-accent);font-size:10px}@keyframes pakethub-expand{from{opacity:0;transform:translateY(-5px)}to{opacity:1;transform:translateY(0)}}.manage-modal{width:min(520px,100%);border-radius:20px;background:var(--card-background-color);box-shadow:0 24px 80px rgba(0,0,0,.45)}.manage-modal header{display:flex;align-items:center;justify-content:space-between;padding:16px 18px;border-bottom:1px solid var(--divider-color)}.manage-modal form{display:grid;gap:14px;padding:18px}.manage-modal label{display:grid;gap:6px;font-size:12px;color:var(--secondary-text-color)}.manage-modal input{box-sizing:border-box;width:100%;min-height:44px;padding:9px 11px;border:1px solid var(--divider-color);border-radius:11px;background:var(--secondary-background-color);color:var(--primary-text-color);font:inherit}.form-actions{display:flex;justify-content:flex-end;gap:8px}.form-actions button{padding:9px 14px;border-radius:10px;background:var(--secondary-background-color)}.form-actions .primary{background:var(--primary-color);color:var(--text-primary-color,#fff)}.form-message{min-height:18px;color:var(--error-color,#db4437);font-size:12px}@container(max-width:560px){.toolbar{flex-direction:column;align-items:stretch}.sort-control{justify-content:center}.sort-control select{max-width:none;flex:1}.filter-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))}.status-filter,.favorites-filter{min-height:40px;padding:0 7px}}@container(max-width:390px){.filter-row{grid-template-columns:repeat(2,minmax(0,1fr))}}</style><ha-card><div class="header"><div class="header-top"><div class="brand"><div class="brand-icon"><ha-icon icon="mdi:package-variant-closed"></ha-icon></div><div><div class="title">${this._escape(this._config.title || "PaketHub")}</div><div class="version">Dashboard v${CARD_VERSION}</div></div></div><div class="header-actions"><button type="button" aria-label="${this._escape(this._t("add"))}" class="add-package"><ha-icon icon="mdi:plus"></ha-icon></button><button type="button" aria-label="${this._escape(this._t("refresh"))}" class="refresh ${this._refreshing ? "refreshing" : ""}" ${this._refreshing ? "disabled" : ""}><ha-icon icon="mdi:refresh"></ha-icon></button></div></div>
      <div class="summary"><button type="button" class="summary-item ${this._statusFilter==="active"&&!this._favoritesOnly?"active":""}" data-summary-filter="active"><span class="summary-value">${summary.active}</span><span class="summary-label">${this._escape(this._t("active"))}</span></button><button type="button" class="summary-item ${this._statusFilter==="transit"&&!this._favoritesOnly?"active":""}" data-summary-filter="transit"><span class="summary-value">${summary.transit + summary.delivery}</span><span class="summary-label">${this._escape(this._t("packages_transit"))}</span></button><button type="button" class="summary-item ${this._statusFilter==="delivered"&&!this._favoritesOnly?"active":""}" data-summary-filter="delivered"><span class="summary-value">${summary.delivered}</span><span class="summary-label">${this._escape(this._t("delivered"))}</span></button><button type="button" class="summary-item ${this._statusFilter==="problems"&&!this._favoritesOnly?"active":""}" data-summary-filter="problems"><span class="summary-value">${summary.error}</span><span class="summary-label">${this._escape(this._t("problems"))}</span></button></div><div class="toolbar"><div class="search"><ha-icon icon="mdi:magnify"></ha-icon><input type="search" placeholder="${this._escape(this._t("search"))}" value="${this._escape(this._query)}"></div><label class="sort-control"><span class="sr-only">${this._escape(this._t("sort"))}</span><ha-icon icon="mdi:sort"></ha-icon><select aria-label="${this._escape(this._t("sort"))}"><option value="status" ${this._config.sort_by==="status"?"selected":""}>${this._escape(this._t("sort_status"))}</option><option value="eta" ${this._config.sort_by==="eta"?"selected":""}>${this._escape(this._t("sort_eta"))}</option><option value="updated" ${this._config.sort_by==="updated"?"selected":""}>${this._escape(this._t("sort_updated"))}</option><option value="name" ${this._config.sort_by==="name"?"selected":""}>${this._escape(this._t("sort_name"))}</option><option value="carrier" ${this._config.sort_by==="carrier"?"selected":""}>${this._escape(this._t("sort_carrier"))}</option></select></label></div><div class="filter-row" role="group" aria-label="Statusfilter"><button class="status-filter ${this._statusFilter==="all"?"active":""}" data-filter="all">${this._escape(this._t("all"))}</button><button class="status-filter ${this._statusFilter==="active"?"active":""}" data-filter="active">${this._escape(this._t("active_filter"))}</button><button class="status-filter ${this._statusFilter==="transit"?"active":""}" data-filter="transit">${this._escape(this._t("transit_filter"))}</button><button class="status-filter delivered-filter ${this._statusFilter==="delivered"?"active":""}" data-filter="delivered"><ha-icon icon="mdi:package-variant-closed-check"></ha-icon>${this._escape(this._t("delivered_filter"))}<span>${summary.delivered}</span></button><button class="status-filter ${this._statusFilter==="problems"?"active":""}" data-filter="problems">${this._escape(this._t("problems_filter"))}</button><button class="favorites-filter ${this._favoritesOnly?"active":""}"><ha-icon icon="mdi:star"></ha-icon>${this._escape(this._t("favorites"))}</button></div><div class="last-sync">${this._escape(this._relative(this._lastSync()?.state))}</div></div><div class="content">${body}<div class="empty search-empty" hidden><ha-icon icon="mdi:magnify-close"></ha-icon><div><strong>${this._escape(this._lang() === "de" ? "Keine Treffer" : "No matches")}</strong><span>${this._escape(this._lang() === "de" ? "Passe den Suchbegriff an." : "Try a different search term.")}</span></div></div></div></ha-card>`;
    ...
    this.shadowRoot.querySelectorAll("[data-summary-filter]").forEach(button=>button.addEventListener("click",()=>{this._statusFilter=button.dataset.summaryFilter||"active";this._favoritesOnly=false;this._signature="";this._render();}));
    this.shadowRoot.querySelector(".sort-control select")?.addEventListener("change",event=>{this._config={...this._config,sort_by:event.currentTarget.value};this._signature="";this._render();});
    ...
    this.shadowRoot.querySelectorAll('[data-action="timeline-more"]').forEach(el=>el.addEventListener("click",e=>{e.stopPropagation();const item=packages[Number(el.dataset.index)];const n=String(item?.tracking_number||"");this._timelineAll.has(n)?this._timelineAll.delete(n):this._timelineAll.add(n);this._signature="";this._render();}));
